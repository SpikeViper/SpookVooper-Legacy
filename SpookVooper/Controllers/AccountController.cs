...
using SpookVooper.Controllers;
using SpookVooper.Web.Models.AccountViewModels;
...
namespace SpookVooper.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public static string forbidden { get; set; }
        ...
        [Authorize]
        public async Task<IActionResult> ConnectMinecraft()
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
            {
            ...
            string key = _connectionHandler.GenerateAPIKey(user.Id);
        ...
        [Authorize]
        public async Task<IActionResult> ConnectTwitch()
        {
            var user = await _userManager.GetUserAsync(User);
            if (user == null)
            {
        ...
        public async Task<IActionResult> ViewAPIKey()
        {
            ...
            return View((user.Api_Key, user.api_use_count));
        }
        ...
        public async Task<IActionResult> Login(LoginViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError(string.Empty, "Invalid login attempt. If you set a username, please log in using that!");
                    return View(model);
                }
            }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                if (_userManager.Users.Any(u => u.UserName.ToLower() == model.Username.ToLower() || 
                                                u.Email.ToLower() == model.Email.ToLower()))
                {
                    ModelState.AddModelError(string.Empty, "That username or email has already been used. Please try a different one.");
                    return View(model);
                }

                var user = new User { Id = "u-" + Guid.NewGuid().ToString(), UserName = model.Username, Email = model.Email, Api_Key = Guid.NewGuid().ToString() };
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Console.WriteLine($"{user.Email} forgot their password.");
